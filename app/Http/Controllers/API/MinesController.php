...
class MinesController extends Controller
{
        public function index(Request $request)
    {
        try {
            ...
            $query = Mine::with(['deliveryOrders' => function($query) {
                $query->select('id', 'do_number', 'mine_id', 'status','issue_date');
            }]);
        ...
        if ($request->filled('search')) {
            $query->where('mine_name', 'like', '%' . $request->search . '%');
        }

        // Pagination inputs
        $page  = $request->params["page"];      // page starts from 1
        $limit = $request->params["offset"];           // offset = limit
        ...
        $mines = $query->orderBy('mine_name', 'asc')
            ->offset($offset)
            ->limit($limit)
            ->get();
        ...
        return response()->json([
            'success' => false,
            'message' => 'Failed to retrieve mines',
        ], 500);
    }
}

    /**
     * Get mines list for dropdown (id and name only)
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Http\Response
     */
    public function getMineList(Request $request)
    {
        try {
            $query = Mine::select('id', 'mine_name');
            ...
            // Search by mine name if provided
            if ($request->has('search') && !empty($request->search)) {
                $query->where('mine_name', 'like', '%' . $request->search . '%');
            }
            
            $mines = $query->orderBy('mine_name', 'asc')->get();
            
            return response()->json([
                'success' => true,
                'data' => $mines,
                'message' => 'Mine list retrieved successfully'
            ], 200);
            
        } catch (\Exception $e) {
            Log::error('Mine dropdown list error', [
                'error' => $e->getMessage(),
                'request' => $request->all()
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Failed to retrieve mine list',
    ...
    public function store(Request $request){
        ...
        $validator = Validator::make($request->all(), [
            'mine_name' => 'required|string|unique:mines,mine_name',
        ...
        $exists = Mine::where('mine_name', $request->mine_name)->exists();

        if ($exists) {
            return response()->json(["code"=>500,'message' => 'The mine name already.',"status"=>false]);
        }

        $data = $request->all(); // Copy request data
        $mineRecord = Mine::create($data);
        $lastId = $mineRecord->id;
        return response()->json(["code"=>200,'message' => 'Mine created successfully']);
    ...
    public function getMinesList(Request $request)
    {
        $page   = max(1, (int) $request->get('page', 1));
        $offset = max(1, (int) $request->get('offset', 10));
        $name   = $request->get('name');

        $query = Mine::where('status', 'active')
            ->when(!empty($name), function ($q) use ($name) {
                $q->where('mine_name', 'LIKE', "%{$name}%");
            })
            ->latest(); // 🔥 latest first
   ...
   public function sync(Request $request){
    ...
    public function showMine($id)
    {
        try {
            ...
            return response()->json([
                "code" => 200,
                'message' => 'Mine retrieved successfully',
                "data" => $mine
            ]);
            ...
            return response()->json([
                "code" => 500,
                'message' => 'Failed to retrieve mine',
                "data" => null
            ], 500);
        }
    }
    ...
    /**
     * Update the specified mine in storage.
     */
    public function updateMine(Request $request, $id)
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                'mine_name' => 'sometimes|required|string|unique:mines,mine_name,' . $id,
            ...
            // Check for duplicate mine name if being updated
            if ($request->has('mine_name') && $request->mine_name !== $mine->mine_name) {
                $exists = Mine::where('mine_name', $request->mine_name)->where('id', '!=', $id)->exists();
                if ($exists) {
                    return response()->json([
                        "code" => 500,
                        'message' => 'The mine name already exists.',
                        "status" => false
                    ]);
                }
            }

            $mine->update($request->all());
    ...
    /**
     * Remove the specified mine from storage.
     */
