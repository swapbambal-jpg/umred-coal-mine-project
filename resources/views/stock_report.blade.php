        ...
        @foreach($orders ?? [] as $order)
            ...
            <tr>
                ...
                <td style="text-align:center"> <a href="javascript:void(0);" 
                           ...
                           data-price="{{ $order['id'] }}" 
                           style="width:100%; background: #343a40" onclick="editProduct('{{ $order["id"] }}')">View Items </a>
                </td>
...
document.getElementById("export").addEventListener("click", function () {
    ...
    let csv = "SKU,Table,Item name,Total amount, Partial Payment, Payment Method\n";
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        let method          = $("<div>").html(row[5]).text().trim();   // Removes <h2>

        csv += `"${sku}","${product}","${price}","${sellPrice}","${qty}","${method}"\n`;
    }
