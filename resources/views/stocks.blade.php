...
<div class="wrap" style="margin:92px 60px !important">
  <div class="card">
    ...
    <div class="summary-actions">
        <div class="row">
             <div class="col-md-4">
                  <select class="form-select form-control" name="categoryFilter" id="categoryFilter" onchange="getProductByCat(this.value)" >
            ...
            <div class="col-md-4"><button class="btn btn-success" id="show_form" style="float: right;">➕ Add Product</button></div>
      ...
      <tbody id="tbody">
        ...
        <?php $product['image']= !empty($product['image'])?$product['image']:"img-7.png";?>
            <tr>
                <td>P-{{ $product["id"] }}</td>
                <td>{{ $product["name"] }}</td>
                <td> @if(!empty($product['counter_product_stocks']))
            <div style="display:flex; gap:10px; flex-wrap:wrap;">
                @foreach($product['counter_product_stocks'] as $price)
...
<script>
    $(document).ready(function () {
        ...
        if (!categoryFilter || categoryFilter === "") {
            ...
            window.location = "{{ url('/stocks') }}/"+firstOption+"/"+restTypeId;
    ...
    function getProductByCat(catId){
        let restTypeId = "{{ $restaurant_type }}"; // Get authenticated user ID
        window.location = "{{ url('/stocks') }}/"+catId+"/"+restTypeId;
    }
